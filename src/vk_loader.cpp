...
// Convert OpenGL filters to Vulkan
...
std::optional<std::shared_ptr<LoadedGLTF>> loadGltf(VulkanEngine* engine, std::string_view filePath)
{
    ...
    // create buffer to hold the material constants
    ...
    for (fastgltf::Material& mat : gltf.materials) {
        ...
        materialResources.metalRoughImage = engine->_whiteImage; // FIXME when is this replaced???
        ...
        // grab textures from gltf file (optional in gltf materials)
    ...
    // load all nodes with local transform and set their meshes
    for (fastgltf::Node& node : gltf.nodes) {
        ...
        std::visit(fastgltf::visitor{ [&](fastgltf::Node::TransformMatrix matrix) {
            ...
            node.transform); // node.transform is either 3-component TRS, or single-array TransformMatrix
    }

    // run loop again to setup hierarchy
    ...
    for (auto& node : nodes) {
        if (node->parent.lock() == nullptr) {
            ...
            node->refreshTransform(glm::mat4{ 1.f }); // recursive update of txform
        }
    }
    return scene;
}
...
void LoadedGLTF::clearAll() {
    for (auto& samp : samplers) {
        vkDestroySampler(creator->_device, samp, nullptr);
    }
